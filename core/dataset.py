...
class TinyImageNet(torch.utils.data.Dataset):
    def __init__(self, root, train=True, transform=None):
        ...
        if train:
            ...
            label_map = torchvision.datasets.ImageFolder(
                pth.join(self._root, "train")
            ).class_to_idx
...
def ds_monkey_patch_targets(dataset):
    ...
    targets = None
    if isinstance(
        dataset, (torchvision.datasets.CIFAR10, torchvision.datasets.CIFAR100)
    ):
        if dataset.train:
...
def ds_subset(dataset, indices):
    ...
    assert hasattr(
        dataset, "targets"
    ), "dataset is expected to have 'targets' attribute"
    assert len(set(indices)) == len(
        indices
    ), "indices is expected to have unique entries"
    assert all(
        isinstance(i, int) for i in indices
    ), "indices is expected to have int entries."
    assert min(indices) >= 0 and max(indices) <= len(
        dataset
    ), "indices is expected to have entries in [0, len(dataset)]."
...
def ds_random_subset(dataset, percentage=None, absolute_size=None, replace=False):
    ...
    n_samples = (
        int(percentage * len(dataset)) if percentage is not None else absolute_size
    )
    indices = np.random.choice(list(range(len(dataset))), n_samples, replace=replace)
...
def ds_label_filter(dataset, labels):
    ...
    assert len(set(labels)) == len(
        labels
    ), "labels is expected to have unique elements."
    assert hasattr(
        dataset, "targets"
    ), "dataset is expected to have 'targets' attribute"
    assert set(labels) <= set(
        dataset.targets
    ), "labels is expected to contain only valid labels of dataset"
