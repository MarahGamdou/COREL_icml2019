...
class DCGEncDec(nn.Module):
    ...
    def __init__(
        ...
        *ignore,
        ...
        enc_dim = torch.tensor(
            list(self.enc_conv(torch.randn(10, *input_config)).size()[1:])
        )
        ...
        self.enc_fc = mynn.IndependentBranchesLinear(
            enc_dim.prod(), self.out_features_branch, self.n_branches
        )

        # Create a linear view
        self.enc = nn.Sequential(self.enc_conv, mynn.LinearView(), self.enc_fc)

        # Unfold the independent linear branches
        self.dec_fc = mynn.IndependentBranchesLinear(
            self.latent_dim, int(enc_dim.prod() / self.n_branches), self.n_branches
        )
        ...
        self.dec = nn.Sequential(self.dec_fc, self.dec_convt)
