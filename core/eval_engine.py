...
def evaluate(
    model, ds_train, ds_test, kde_factory, train_subset_size=1.0, device="cpu"
):
    ...
    for inlier_class in range(n_classes):
        ...
        ds_train_inlier = ds_random_subset(
            ds_train_inlier, absolute_size=train_subset_size, replace=False
        )
...
class ScorerFactory:
    def __init__(self, *ignore, scorer_id, input_dim, subspace_dim, scorer_kwargs={}):
    ...
    def __call__(self):
        return CountScorerProduct(
            self.input_dim, self.subspace_dim, **self.scorer_kwargs
        )


def evaluate_backbone_with_scorer(
    eval_config, backbone_trn_result, output_folder, device=None
):
    ...
    trn = dataset_factory(dataset=eval_config["dataset"], train=True, subset_ratio=None)

    # Get whole test dataset
    tst = dataset_factory(
        dataset=eval_config["dataset"], train=False, subset_ratio=None
    )
    ...
    sc_factory = ScorerFactory(
        scorer_id=eval_config["scorer"]["id"],
        input_dim=model.latent_dim,
        subspace_dim=model.latent_dim
        // backbone_trn_result.config["model_args"]["kwargs"]["latent_config"][
            "n_branches"
        ],
...
def evaluate_backbones_on_eval_config_grid(
    backbone_result_folder, eval_config_grid, output_folder, device
):
    ...
    for bbr in backbone_results:
        for cfg in eval_configs:
            print(i, "/", n)
            evaluate_backbone_with_scorer(cfg, bbr, output_folder, device=device)
            i += 1
            gc.collect()
