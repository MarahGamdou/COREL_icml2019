...
class TruncatedExponentialL1KDM:
    def __init__(
        ...
        scaling=1,
        truncation_radius=1,
        kernel_aggregation=lambda x: x.sum(1),
    ):
        ...
        self._norm_factor = (
            2 * scaling * (1 - math.exp(-(truncation_radius / scaling)))
        ) ** (-dimension)
...
class CountScorerProduct:
    def __init__(
        self, dimension, sub_space_dim, truncation_radius=1, aggregation="sum"
    ):
    ...
    def score_sample(self, X):
        ...
        for i in range(0, self._dimension, self._sub_space_dim):
            ...
            D = distance_matrix_l1(
                X[:, i : (i + self._sub_space_dim)],
                self._train_data[:, i : (i + self._sub_space_dim)],
            )
