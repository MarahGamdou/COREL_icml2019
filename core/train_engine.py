...
def get_keychain_value(d, key_chain=None, allowed_values=(list,)):
    ...
    if not isinstance(d, dict):
        if allowed_values is not None:
            assert isinstance(
                d, allowed_values
            ), "Value needs to be of type {}!".format(allowed_values)
        yield key_chain, d
    else:
        for k, v in d.items():
            yield from get_keychain_value(
                v, key_chain + [k], allowed_values=allowed_values
            )
...
model_mapping = {"DCGEncDec": autoencoder.DCGEncDec}
...
def train(root_folder, config):
    ...
    optim = Adam(model.parameters(), lr=train_args["learning_rate"])

    ds = dataset_factory(**config["data_args"])
    dl = DataLoader(
        ds, batch_size=train_args["batch_size"], shuffle=True, drop_last=True
    )
    ...
    for epoch in range(1, train_args["n_epochs"] + 1):
        ...
        for x, _ in dl:
            ...
            for i in range(0, latent_dim, branch_siz):
                pers = vr_l1_persistence(
                    ...
                    0
                ]  # [0][0] gives non-essential in H_0
            ...
            loss = (
                train_args["rec_loss_w"] * rec_loss
        ...
        print(
            "{}: rec_loss: {:.4f} | top_loss: {:.4f}".format(
                epoch,
                np.array(
                    log["rec_loss"][-int(len(ds) / train_args["batch_size"]) :]
                ).mean()
                * train_args["rec_loss_w"],
                np.array(
                    log["top_loss"][-int(len(ds) / train_args["batch_size"]) :]
                ).mean()
                * train_args["top_loss_w"],
            )
        )
...
class BackboneTrainResultFolder:
    ...
    def _load_train_results_from_folder(self, root):
        ...
        for config, model, log in zip(f_config, f_model, f_log):
            assert (
                len(set((config.split(".")[0], model.split(".")[0], log.split(".")[0])))
                == 1
            )
            base = config.split(".")[0]
    ...
    def where_unique(self, **kwargs):
        ...
        assert (
            len(res) == 1
        ), "Your query was not unique. {} elements were found.".format(len(res))
        return res[0]
